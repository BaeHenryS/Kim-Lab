...
if isfield(scan,'consts') && ~isempty(scan.consts)
    if ~isfield(scan.consts,'set')  % default value is 1
...
%apply configfn's on scan
%configfn has format smscan.configfn as a vector
%smscan.configfn has fields fn, args
%configfn must use a scan as its first parameter
...
for i = 1:totpoints   
    ...
    xt = x;  
%     Change added by Patrick Maher 11/22/11
...
%                 Change end
        ...
        %%%%%%%%%%%%%%%%%%% added stuff starting here
        if any(autochan) && length(scandef(j).setchan) == 1
            instchan=smdata.channels(scandef(j).setchan).instchan;
            dummyautochan = isfield(smdata.inst(instchan(1, 1)),'faulty') &&...
                ~isempty(smdata.inst(instchan(1, 1)).faulty) &&...
                smdata.inst(instchan(1, 1)).faulty;
        else
            dummyautochan=false;
        end
        
        if dummyautochan %one set channel which has neg ramprate AND is tagged as dummy
            if count(j) == 1;
                initial_val = val;
            else
                while true %wait for break only
                    curr_val= smdata.inst(instchan(1, 1)).cntrlfn([instchan(1, :), 0]);
                    %break when the inst passes the target value (vals2)
                    %val should be scalar since length(scandef(j).setchan) = 1
                    if (initial_val<=val && curr_val>=val) ||...
                            (initial_val>=val && curr_val<=val)
                        break
                    else pause(.5)
                    end
                end
            ...
            pause(tp);
            if tp<0 && any(autochan)
                warning('Data acquisition lagging behing self ramping channels')
            end
        end
        ...
        % trigger after waiting for first point. autochans (rate<0) trigger
        % HERE
