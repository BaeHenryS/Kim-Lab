function [val, rate] = SR860(ic, val, rate,ctrl)
        ...
        switch ic(3)
            case 0  % get              
                ...
                fprintf(smdata.inst(ic(1)).data.inst, 'TRCB? %d, %d, %d', ...
                    [ic(2)-14, smdata.inst(ic(1)).data.currsamp, npts]); %changed by Carlos
%                   [ic(2)-14, smdata.inst(ic(1)).data.currsamp+[0, npts]]);
                val = fread(smdata.inst(ic(1)).data.inst, npts, 'single'); %note: unlike all other communications, this is NOT in ASCII
            ...
            case 5
                ...
                fprintf(smdata.inst(ic(1)).data.inst, 'REST; SEND 0; TSTR 1; SRAT %i', n); %Carlos-changed SEND to 0 from 1
                %else
