...
function loopvars_eth_Callback(hObject,eventdata,i,j)
    ...
    elseif j==2  %Ramptime being changed
        ...
        makelooppanels
    elseif j==5  %Ramp rate being changed
        val = str2double(get(obj,'String'));
        if val %check that its non-zero
            smscan.loops(i).ramptime=val^-1;
        end
        makelooppanels
...
function loopcvars_eth_Callback(hObject,eventdata,i,j,k)
    ...
    elseif k==5 % Change the range of the channel
        ...
        rng=smscan.loops(i).setchanranges{j};  %why is this here!?!??!
    ...
    elseif k==7 % change the rate also *FOR ALL CHANNELS IN LOOP*
        val = str2double(get(obj,'String'));
        smscan.loops(i).ramptime=(smscan.loops(i).setchanranges{j}(2)-smscan.loops(i).setchanranges{j}(1))/(smscan.loops(i).npoints-1)/val;
        set(smaux.smgui.loopvars_eth(i,2),'String',smscan.loops(i).ramptime); %update ramptime
        set(smaux.smgui.loopvars_eth(i,5),'String',(smscan.loops(i).ramptime)^-1); %update ramprate
        for c=1:smscan.loops(i).numchans %update all channels in loop
            makeloopchannelset(i,c)
        end
    end
    makeloopchannelset(i,j);
...
function loopcvarsLOCKT_eth_Callback(hObject,eventdata,i,j,k)
    ...
    elseif k==7 % change the rate also *FOR ALL CHANNELS IN LOOP*
        val = str2double(get(obj,'String'));
        smscan.loops(i).ramptime=(smscan.loops(i).rng(2)-smscan.loops(i).rng(1))/(smscan.loops(i).npoints-1)/val;
        set(smaux.smgui.loopvars_eth(i,2),'String',smscan.loops(i).ramptime); %update ramptime
        set(smaux.smgui.loopvars_eth(i,5),'String',(smscan.loops(i).ramptime)^-1); %update ramprate
        for c=1:smscan.loops(i).numchans %update all channels in loop
            makeloopchannelset(i,c)
...
function UpdateConstants(varargin)
    global smaux smscan;
    %insert fix here PM 10/6/11--works!
...
function makelooppanels(varargin)
    ...
    panel1position = [180 650 700 290];
    ...
    for i = (length(smaux.smgui.loop_panels_ph)+1):numloops % display data from existing loops
        smaux.smgui.loop_panels_ph(i) = uipanel('Parent',smaux.smgui.nullpanel,...
            'Units','pixels',...
            'Position',panel1position+[0*(i) -350*(i) -0*(i) 0]);
        ...
        smaux.smgui.loopvars_sth(i,2) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
            ...
            'String','Step time:',...
            'HorizontalAlignment','right',...
            'Position',[315 247 55 20]);
        smaux.smgui.loopvars_eth(i,2) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
            'Style','edit',...
            'String',smscan.loops(i).ramptime,...
            'HorizontalAlignment','center',...
            'Position',[375 252 55 20],...
            'Callback',{@loopvars_eth_Callback,i,2});
        
        %Step Rate
        smaux.smgui.loopvars_sth(i,5) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
            'Style','text',...
            'String','Point rate:',...
            'HorizontalAlignment','right',...
            'Position',[430 247 55 20]);
        smaux.smgui.loopvars_eth(i,5) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
            'Style','edit',...
            'String',(smscan.loops(i).ramptime)^-1,...
            'HorizontalAlignment','center',...
            'Position',[490 252 55 20],...
            'Callback',{@loopvars_eth_Callback,i,5});
...
function makeloopchannelset(i,j) 
    ...
    ws = [100 40 25 25 40 30 30]; % widths-text
    we = [100 40 40 40 40 40 40]; % widths-edit
    h = [20 20 20 20 20 20 20 20 20 20 50]; % heights
    s = 95; %spacing
    subpos=[110 205 290 360 430 530 610];
    ...
    if isfield(smscan.loops(i), 'setchanranges')
        ...
        %steprate
        smaux.smgui.loopcvars_sth(i,j,7) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
            'Style','text',...
            'String','Rate:',...
            'HorizontalAlignment','right');
        smaux.smgui.loopcvars_eth(i,j,7) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
            'Style','edit',...
            'String',(smscan.loops(i).setchanranges{j}(2)-smscan.loops(i).setchanranges{j}(1))/(smscan.loops(i).npoints-1)*smscan.loops(i).ramptime^-1,...
            'HorizontalAlignment','center',...
            'Callback',{@loopcvars_eth_Callback,i,j,7});

        for k=2:7
            set(smaux.smgui.loopcvars_sth(i,j,k),'Position',pos+[subpos(k) -5 ws(k) h(k)]);
            set(smaux.smgui.loopcvars_eth(i,j,k),'Position',pos+[subpos(k)+ws(k)+5 0 we(k) h(k)]);            
        end
    ...
    elseif j==1 %first setchan in locked trafofn mode, loop range != [0 1];
        ...
        smaux.smgui.loopcvars_eth(i,j,7) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
            'Style','edit',...
            'String',(smscan.loops(i).rng(2)-smscan.loops(i).rng(1))/(smscan.loops(i).npoints-1)*smscan.loops(i).ramptime^-1,...
            'HorizontalAlignment','center',...
            'Callback',{@loopcvarsLOCKT_eth_Callback,i,j,7});
...
function makeloopgetchans(i)
    ...
    %smaux.smgui.loopvars_getchans_pmh=[]; %commented so all loops editable
    channelnames = {};
    for k = 1:length(smdata.channels)
        channelnames{k}=smdata.channels(k).name;
    end
    ...
    for j=1:numgetchans
        ...
        smaux.smgui.loopvars_getchans_pmh(i,j) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
            ...
            'Value',chanval,...
            'HorizontalAlignment','center',...
            'Position',[60+90*(mod((j-1),7)) 90-30*floor((j-1)/7) 80 20],...
    ...
    smaux.smgui.loopvars_getchans_pmh(i,j+1) = uicontrol('Parent',smaux.smgui.loop_panels_ph(i),...
        ...
        'Value',1,...
        'HorizontalAlignment','center',...
        'Position',[60+90*(mod(j,7)) 90-30*floor(j/7) 80 20],...
